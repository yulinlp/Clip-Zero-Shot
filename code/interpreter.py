...
class Environment:
    def __init__(self, image: Image, boxes: List[Box], executor: "Executor" = None, freeform_boxes: bool = False, image_name: str = None, file_name: str = None, is_dynamic = False):
        self.image = image
        self.boxes = boxes
        self.executor = executor  # An object or callback that can query CLIP with captions/images.（clip对象）
    ...
    def filter(self,
        ...
        # 返回一个新的分布，反映“标题”描述每个框内容的可能性。（利用clip）
        ...
        result_partial = self.executor(caption, self.image, boxes, image_name=self.image_name, file_name=self.file_name, is_dynamic = self.is_dynamic)
        # 修改，加了Q
        # 改完批量，下面这块就没用了
        # if self.corpus_Q is not None: 
        #     bias = [self.executor(sentence, self.image, boxes, image_name=self.image_name, file_name=self.file_name) for sentence in self.corpus_Q]
        #     result_partial -= sum(bias) / len(bias)
    ...
    def filter_area(self, area_threshold: float) -> np.ndarray:
        ...
        # 返回一个新的分布，其中所有框的面积（图像的一部分）都小于阈值。
